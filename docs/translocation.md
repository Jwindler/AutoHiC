...
​		从各个分辨率进行查找插入位置，直到返回位置为一个ctg时，就结束查找
...
由于修改文件存在一定的冲突，目前采用如下方式：

1. 首先全部易位切割ctg，然后查找插入位置，记录要插入的ctg名字和方向
2. 等全部错误都切割完毕后，再重新查找错误区间内的ctg，然后移动
